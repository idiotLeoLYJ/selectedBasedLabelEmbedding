...
from model.configuration_roberta import RobertaConfig  # 这是自己定义的
...
@add_start_docstrings("""RoBERTa Model with a `l2 norm` head on top. """, ROBERTA_START_DOCSTRING)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            for ix, (label_sample, context_sample, label) in enumerate(
                ...
                # for ix, context_sample in enumerate(last_layer_contextual_hidden):
                label_sample = last_layer_hidden[ix]
                # label_index=labels_index[ix]
                ...
                # loss_contra=loss_fct(torch.div(label_sample[1].view(-1), torch.norm(label_sample[1], p=2)),
                #                          torch.div(context_sample.view(-1), torch.norm(context_sample, p=2)))
                # loss_neural=loss_fct(torch.div(label_sample[2].view(-1), torch.norm(label_sample[2], p=2)),
                #                          torch.div(context_sample.view(-1), torch.norm(context_sample, p=2)))
                # loss_entail=loss_fct(torch.div(label_sample[3].view(-1), torch.norm(label_sample[3], p=2)),
                #                          torch.div(context_sample.view(-1), torch.norm(context_sample, p=2)))
